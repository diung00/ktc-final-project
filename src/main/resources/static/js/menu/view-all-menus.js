...
async function loadRestaurantData() {
    ...
    try {
        ...
        document.getElementById('restaurant-name').innerText = restaurantData.name;
    
        const restaurantId = restaurantData.id; // Get restaurantId
        localStorage.setItem('restaurantId', restaurantId); //save into local storage
    ...
    .then(menuData => {
        ...
        menuData.forEach(menu => {
            ...
            menuRow.innerHTML = `
                ...
                <td>${menu.menuStatus == 'AVAILABLE' ? '판패중' : '품절'}</td>
        ...
        document.querySelectorAll('.update-menu-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                location.href = `/views/restaurant-update-menu?menuId=${menuId}`;
            });
        });
        //Menu delete button
