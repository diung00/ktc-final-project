...
async function fetchOrders() {
    try {
        ...
        if (orders.length > 0) {
            displayOrderDetails(orders[0].id);
        } 
    } catch (error) {
...
function displayOrders(orders) {
    ...
    ordersContainer.innerHTML = ''; // Clear any existing orders
    if (orders.length === 0) {
    ...
    orders.forEach(order => {
        ...
        orderElement.innerText = `Order #${order.id} - ${order.restaurantName} - ${order.orderStatus}`;
